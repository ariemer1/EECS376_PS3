...
const double RADIUS_LEFT_TURN = 0.1;
...
struct Averages { //makes returning averages easier
        double avgX;
        double avgY;
	};
...
void odomCallback(const nav_msgs::Odometry& odom_msg) {
	...
	while (abs(g_phi) >= 2*M_PI || g_phi < 0) {//checks to make sure g phi is always between 0 and 2pi
	    if(g_phi > 0) {
	        g_phi -= 2*M_PI;
	    } else {
	        g_phi += 2*M_PI;
...
//on very first call, set up some values that will be re-used in subsequent calls
    if (g_index_front < 0) { //for first message received, set up the desired index of LIDAR range to eval
...
//gets the x & Y values from the ping and angle
double pingX(double dist, double angle) {
    return cos(angle)*dist;
}
double pingY(double dist, double angle) {
    return sin(angle)*dist;
}

Averages averagePings(int avgNumber, int firstPoint) {//averages laserscan index in front of the firstPoint
	double sumX = 0.0;
    double sumY = 0.0;

    
    for (int i = firstPoint; i > (firstPoint - avgNumber); i-- ) {//have to go backwards here so it goes clockwise
	    sumX += pingX(g_laser_scan.ranges[i], M_PI - (i-g_index_90deg_right)*g_angle_increment);
        sumY += pingY(g_laser_scan.ranges[i], M_PI - (i-g_index_90deg_right)*g_angle_increment);
    }
    Averages result = {(sumX / avgNumber),(sumY / avgNumber)};
    return result;
}

//Returns the angle that the wall is facing assuming that counter clowckwise is postive
double leftWallAngle() {
    double wallAngle = 0.0;
    int leftPing = g_index_90deg_left;
    int leftPingAbove = g_index_90deg_left - (int)((M_PI/4)/g_angle_increment); 

    Averages AvgLeft;
    Averages AvgLeftAbove;
    AvgLeft   = averagePings(30, leftPing);
    AvgLeftAbove = averagePings(30, leftPingAbove);
    ROS_INFO("Average Left: %f, %f",AvgLeft.avgX, AvgLeft.avgY);
    ROS_INFO("Average Left Up: %f, %f",AvgLeftAbove.avgX, AvgLeftAbove.avgY);
    double distLeft   = sqrt(pow(AvgLeft.avgX,2) + pow(AvgLeft.avgY,2)); //distance to left wall
    double distLeftAbove = sqrt(pow(AvgLeftAbove.avgX,2) + pow(AvgLeftAbove.avgY,2)); //Distance to higher up on the wall

    if (distLeftAbove - distLeft > WALL_FOLLOW_RADIUS) { //don't turn if the distLeftAbove is off the wall
        ros::spinOnce(); //update g_phi
        ROS_INFO("No Path");
        return 0; //won't turn with no info
    } else {
        ROS_INFO("Path Found");
        double angle = atan((AvgLeftAbove.avgY - AvgLeft.avgY)/(AvgLeftAbove.avgX - AvgLeft.avgX)) - (M_PI/2); //add pi/2 to make 0 vertical
        if(angle < 0) { //makes sure wall angle is always postive
            angle += 2*M_PI;
        }
        return angle;
    }
...
//start by determining what angle the bot is with relation to the wall
                double angleToTurn = leftWallAngle();
                ROS_INFO("Turning %f radians to be parallel", angleToTurn);
                ROS_INFO("Turning to become parallel with the wall");
                double currPhi = g_phi;
                while(abs(abs(g_phi - currPhi) - angleToTurn) >= 0.05) { //spin in place while the robot angle is further than .05 radians from the left wall
                    twist_cmd.angular.z = YAW_RATE;
                    twist_cmd.linear.x = 0;
        	        twist_commander.publish(twist_cmd);
                    ros::spinOnce();
            ...
            twist_cmd.linear.x = SPEED; //move forwards
            twist_cmd.angular.z = 0.0;
        }
    } //loop forever
}
